...
resource "aws_key_pair" "keypair" {
	...
	public_key = "${file("${path.module}/../../keys/prod_keypair.pub")}" 
}

module "bastion" {
	source                    = "../../modules/bastion/"
	vpc_cidr_block            = "${data.terraform_remote_state.prod_vpc.vpc_cidr_block}"
	public_subnet_id          = "${data.terraform_remote_state.prod_vpc.public_subnet_ids[0]}"
	bastion_ami_id            = "${var.bastion_ami_id}"
	aws_key_name              = "${aws_key_pair.keypair.key_name}"
	aws_environment           = "${var.aws_environment}"
	aws_bastion_sgr_name      = "bastion"
	aws_bastion_instance_name = "Prod-BastionHost"
...
module "dcore-alb" {
	source                       = "../../modules/alb/"
	aws_region                   = "${var.aws_region}"
	vpc_id                       = "${data.terraform_remote_state.prod_vpc.vpc_id}"
	vpc_cidr_block               = "${data.terraform_remote_state.prod_vpc.vpc_cidr_block}"
	vpc_name                     = "${var.aws_vpc_name}"
	public_subnet_ids            = ["${data.terraform_remote_state.prod_vpc.public_subnet_ids}"]
	aws_environment              = "${var.aws_environment}"
